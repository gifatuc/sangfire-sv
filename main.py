...
def Rungame():
    ...
    pause = False; pause_count = 0 #0: 실행, 1:정지
    while flag:
        # 현재 시간
        ...
        if pygame.key.get_pressed()[pygame.K_ESCAPE] and pause_count == 1: #정지일때 입력
            pause = False; pause_count = 0  #실행
        elif (pygame.key.get_pressed()[pygame.K_ESCAPE] and pause_count == 0) or pause_count == 1: #실행일때 입력 or 정지 중
            if pause_count == 0: #실행표지라면 정지로
                pause_count = 1
            pause = True #정지
        
        elif pause_count == 0: #실행일때
            p1.move()
            # 플레이어 좌표값
            player_x=p1.loc.x
            player_y=p1.loc.y
